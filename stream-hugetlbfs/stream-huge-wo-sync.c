// This version of stream uses a hugepages to test the functionality
...
// mmap call is just one block of memory os size 3 * STREAM_SIZE. Might be
// able to allocate more than 1 GB using an actual DAX device!
...
int main(int argc, char* argv[]) {
    ...
    // we'll use a hugepage to run stream.
    ...
    int *ptr = mmap((int *) 0x0, 1 << 30, PROT_READ | PROT_WRITE,
